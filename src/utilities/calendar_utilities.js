...
import {DAYNAME_NUMBERS} from "../data/data.jsx";
...
export const addEmptyDaysOfPrevMonth = (startDay) => {
    ...
    let count = DAYNAME_NUMBERS[startDay.toLocaleDateString('nl-BE', {weekday: 'long'}).toLowerCase()]
    if (count !== 1) {
...
export const addEmptyDaysOfNextMonth = (endDay, dateSelected) => {
    ...
    if (lastDayOfMonth(dateSelected).toLocaleDateString('nl-BE', {weekday: 'long'}).toLowerCase() !== 'zondag') {
        for (let i = 0; i < 7 - DAYNAME_NUMBERS[endDay.toLocaleDateString('nl-BE', {weekday: 'long'}).toLowerCase()]; i++) {
